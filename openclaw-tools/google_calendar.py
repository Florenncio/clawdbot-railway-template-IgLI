...
def update_event(event_id, summary=None, start_time=None, end_time=None, location=None, description=None):
...
def delete_event(event_id):
    ...
    try:
        ...
        return json.dumps({
            "status": "success",
            "message": f"Evento {event_id} deletado com sucesso"
        }, indent=2, ensure_ascii=False)
...
def main():
    ...
    parser.add_argument('action', choices=['list', 'create', 'update', 'delete', 'get', 'list_calendars', 'auth'],
    ...
    if args.action == 'auth':
        ...
        if not args.summary or not args.start_time:
            ...
            sys.exit(1)
        print(create_event(args.summary, args.start_time, args.end_time, 
        ...
        if not args.event_id:
            print(json.dumps({
                "status": "error",
                "message": "É necessário fornecer --event-id"
            }), file=sys.stderr)
            sys.exit(1)
        print(update_event(args.event_id, args.summary, args.start_time, 
                          args.end_time, args.location, args.description))
